...
class PaperRunResult:
    ...
    skipped_no_edge: int
    demo_orders_submitted: int = 0
    demo_order_errors: int = 0
    market_errors: int = 0
...
class PaperTrader:
    def __init__(
        ...
        demo_executor: DemoExecutor | None = None,
    ...
    def run_once(self, limit: int) -> PaperRunResult:
        ...
        notes = f"phase2_paper_run series={len(series_tickers)} limit_per_series={limit}" if series_tickers else "phase2_paper_run"
        scan_id = self.store.create_scan(limit, notes=notes)
        totals = {k: 0 for k in ("markets_seen", "signals", "orders", "no_liq", "no_edge", "demo_submitted", "demo_errors", "market_errors")}
        ...
        # recorded as signals but skipped. This is the primary fix for the shadow losses.
    ...
    def _evaluate_market(self, market: dict) -> _Evaluation | None:
        ...
        yes_ask_size = _float_or_zero(market.get("yes_ask_size_fp"))
        no_ask_size = _first_float(
            market.get("no_ask_size_fp"),
            # In Kalshi binary books, buying NO at the NO ask is equivalent to taking the best YES bid.
        ...
        stateless_skip = None
        if not liquid:
            stateless_skip = "no_executable_liquidity"
        elif selected.ev_cents is None:
            stateless_skip = "missing_fee_adjusted_edge"
        ...
        return _Evaluation(signal=signal, parsed=parsed, event_ticker=_event_ticker(parsed.ticker), selected=selected, stateless_skip=stateless_skip)
    ...
    def _fails_uncertainty_gate(self, parsed, sigma: float) -> bool:
        ...
        return not forecast_resolves_strikes(sigma, spacing, self.settings.forecast_uncertainty_gate_ratio)

    def _mark_best_strike_per_event(self, evaluations: list[_Evaluation]) -> None:
        if not self.settings.best_strike_per_event:
            return
        best: dict[str, _Evaluation] = {}
    ...
    def _persist_evaluation(self, scan_id: int, ev: _Evaluation, totals: dict) -> None:
        parsed = ev.parsed
        ...
        if skipped == "no_executable_liquidity":
            totals["no_liq"] += 1
        elif skipped in ("missing_fee_adjusted_edge", "insufficient_fee_adjusted_edge"):
            totals["no_edge"] += 1

        signal_payload = dict(ev.signal)
        signal_payload["skipped_reason"] = skipped
        signal_id = self.store.insert_signal(scan_id, signal_payload)
        totals["signals"] += 1

        if skipped is None and selected.ask_cents is not None and selected.fee_cents is not None:
            local_order_id = self.store.insert_order(scan_id, signal_id, {
                "ticker": parsed.ticker,
                "side": selected.side,
                "quantity": self.quantity,
                ...
                "fee_cents": selected.fee_cents,
                "status": "FILLED",
                "reason": "fee_adjusted_edge",
            })
            self.store.upsert_position(parsed.ticker, selected.side, self.quantity, selected.ask_cents, selected.fee_cents)
            totals["orders"] += 1
            if self.demo_executor is not None:
                if selected.side == "BUY_YES":
                    demo_result = self.demo_executor.buy_yes_fok(
                        scan_id=scan_id,
                        signal_id=signal_id,
                        local_order_id=local_order_id,
                        ticker=parsed.ticker,
                        quantity=self.quantity,
                        price_cents=selected.ask_cents,
                    )
                else:
                    demo_result = self.demo_executor.buy_no_fok(
                        scan_id=scan_id,
                        signal_id=signal_id,
                        local_order_id=local_order_id,
                        ticker=parsed.ticker,
                ...
                if demo_result.error:
                    self.store.mark_order_demo_rejected(local_order_id, demo_result.error)
                    totals["demo_errors"] += 1
                else:
                    totals["demo_submitted"] += 1
...
def _event_ticker(ticker: str) -> str:
    """Group ladder strikes under one event, e.g. KXHIGHCHI-26MAY28-B66.5 -> KXHIGHCHI-26MAY28."""
    parts = ticker.rsplit("-", 1)
    return parts[0] if len(parts) == 2 else ticker


def _is_lookahead(parsed) -> bool:
    """True when the contract's resolution window has already started/elapsed at scan time.

    Same-day (or past) markets let the forecast incorporate already-observed weather, which
    inflates apparent accuracy and will not reproduce live. Such rows are flagged so they can
    be excluded from calibration.
    """
    ...
    if parsed.target_date > today:
        return False
    # Same calendar day: daily aggregates are partly observed; a point-hour is leaked once it passes.
    if parsed.variable == WeatherVariable.POINT_TEMP_F and parsed.target_hour is not None:
        return datetime.now().hour >= parsed.target_hour
    return True
